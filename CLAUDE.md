...
7. **Расписание:** `SCHEDULER_MODE=cron` (часы 00/06/12/18 UTC) или `SCHEDULER_MODE=interval` + `SCHEDULER_INTERVAL_MINUTES=N` (если `SCHEDULER_RUN_ON_STARTUP=true` — тик сразу при старте). `max_instances=1`, `coalesce=true`.
