...
8. **Таймауты:** LLM-вызов 60 сек, обработка одной монеты 600 сек. По таймауту шага монеты — `decision` пишется как `HOLD` + `executed=false` + `not_executed_reason="STEP_TIMEOUT"`, идём к следующей монете.
