...
- pgvector + RAG-поиск по новостям.
...
class CryptoPanicSettings(BaseSettings):# CRYPTOPANIC_*
...
CRYPTOPANIC_NEWS_LIMIT_PER_CRYPTO=20
...
| id | int PK |
| asset | varchar(10) |
| external_id | varchar(128) (id из CryptoPanic) |
...
`GET /v1/posts/?auth_token=...&currencies=BTC&public=true&filter=hot` → top 20 за 24h. Поля: `id`, `title`, `published_at`, `source.title`, `url`.

Дедупликация: по `external_id` (UNIQUE). Если новость уже есть → не зовём embedding/summary повторно.
...
1. PRICE и NEWS ветки — параллельно (asyncio.gather):
   ...
   NEWS branch:
     a. fetch CryptoPanic posts за 24h
...
- `tests/fixtures/cryptopanic_posts.json`.
...
### Фаза 5 — Новости + RAG
- `services/news/cryptopanic.py`, `deduplicator.py`.
